...
export function ensureSession(input: EnsureSessionInput): Session {
  ...
  const existing = db.query<Session, [string]>(
...
export async function addMemory(input: AddMemoryInput): Promise<AddMemoryResult> {
  ...
  ensureSession({ session_id: input.session_id, project_id: input.project_id })
  ...
  const neighbors = db.query<{ id: string; embedding: Buffer }, [string, string]>(
...
export async function searchMemory(input: SearchMemoryInput): Promise<SearchResult[]> {
  ...
  const rows = db.query<
    { id: string; content: string; session_id: string; project_id: string; metadata: string | null; embedding: Buffer },
    input.session_id extends string ? [string, string] : [string]
  >(
    input.session_id
  ...
  ).all(...(input.session_id ? [input.project_id, input.session_id] : [input.project_id]) as any)
  ...
  if (input.expand_graph && results.length > 0) {
    ...
    const neighborMap = new Map<string, { id: string; content: string; score: number }[]>()

    for (const memId of topIds) {
      const edges = db.query<{ neighbor_id: string; score: number }, [string, string]>(
      ...
      for (const edge of edges) {
        ...
        const mem = db.query<{ id: string; content: string }, [string]>(
...
export function getGraph(input: GetGraphInput): GraphResult {
  ...
  function traverse(memId: string, depth: number) {
    ...
    const mem = db.query<{ id: string; content: string; metadata: string | null }, [string]>(
    ...
    const edgeRows = db.query<{ src_id: string; dst_id: string; score: number }, [string, string]>(
...
export function forgetSession(input: ForgetSessionInput): { deleted_count: number } {
  const db = getDb()
